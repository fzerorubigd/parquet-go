...
func TestSchemaParser(t *testing.T) {
	testData := []struct {
		...
		{`message foo { required binary frg_id = 1; required binary client = 2; }`, false},
		{`message foo { optional boolean is_fraud; }`, false},
		{`message foo {
			required binary frg_id (STRING) = 1;
		...
		{`message foo {
			required fixed_len_byte_array(16) frgid (UUID);
		}`, false},
		{`message foo {
			required fixed_len_byte_array frgid;
		}`, true}, // no length provided.
		{`message foo {
			required fixed_len_byte_array(-1) frgid;
...
message foo { }`, false}, // this is necessary because we once had a parser bug when the first character of the parsed text was a newline.
		...
		{`message foo {
			required fixed_len_byte_array(100000000000000000000000000000000000000000000000000000000) frgid (UUID);
